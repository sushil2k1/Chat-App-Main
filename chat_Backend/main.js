...
const deleteExceptFirstFive = async () => {
    const docs = await Message.find({ conversationId: "66f1031199830a2e0726e8cb" }).sort({ _id: 1 }).limit(1).select('_id');
    const idsToKeep = docs.map(doc => doc._id);

    await Message.deleteMany({ _id: { $nin: idsToKeep } });
};

//   deleteExceptFirstFive();
...
app.post('/group',async (req,res)=>{
   try{
    let {members,name,createdBy}=req.body;
    console.log("104",members,name,createdBy);
    let d=await group.create({
        members,createdBy,name
    })
    console.log(d);
    res.status(200).send({mesg:"Error while creating Group"});
   }
   catch(err){
    res.status(400).send({mesg:"Error while creating Group"});
   }
})
...
app.get('/group/:userId',async(req,res)=>{
    try{
        ...
        let con = await group.find({ members: { $in: [userId] } });
        console.log(con);   
