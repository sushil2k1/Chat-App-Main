...
function ChatRoom() {
  ...
  let [usersForGroup, setUsersForGroup] = useState([]);
  ...
  const fetchMessages = async (Id, user1) => {
    try {
      ...
      // Set messages and receiver data
  ...
  let [groupSelection, setGroupSelection] = useState(false);
  ...
  useEffect(() => {
    async function getGroups() {
      let res = await fetch(`http://localhost:2000/group/${user.id}`);
  ...
  async function createGroup() {
    if (groupMember.length < 2) {
      alert("Please add at least two members to create a group.");
      return;
    }
    ...
    // If both validations pass, continue with form submission or other logic.

    // setGroupMember(prevItems => [...prevItems, user.id])
    console.log(1223,groupMember);
    let obj = {
      members: [...groupMember,user.id],
      name: groupName,
      createdBy: user.id
    }
    console.log(obj);
    let res = await fetch('http://localhost:2000/group', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
    ...
    <div className="flex h-full w-full border-w">
      <div className="border-2 w-1/4 p-2 h-screen" id='user'>
        ...
        <div className="p-2 text-lg ">
          ...
          {/* group list */}
          <div className={!g ? "block" : "hidden"}>
            <div className='flex justify-center'>
              <Button name="Create Group" className="w-[180px] bg-blue-600 hover:scale-105 transition-transform transition-shadow duration-300"
                onClick={p => setGroupSelection(true)} />
            </div>
            {groupSelection ? (
              <div className='border-2 rounded-sm p-1 m-2'>
                {usersForGroup.length > 0 ? (

                  usersForGroup.map((user, i) => {
                    return (
                      <div
                        key={user.receiverId}
                        className="flex m-2 border-b border-gray-500 py-2 cursor-pointer hover:bg-gray-200 hover:text-blue-500 hover:scale-105 transform transition-all duration-300"
                      ...
                      >
                        <div className="border flex-col justify-center">
                          <Button
                            className="rounded-[2px] w-[68px] bg-blue-300 mt-3 rounded-md "
                            name="Add"
                            onClick={(event) => {
                              event.stopPropagation();
                              // setGroupMember(prevItems => [...prevItems, user.user.receiverId])
                              setGroupMember(prevItems => {
                                if (!prevItems.includes(user.user.receiverId)) {
                                  return [...prevItems, user.user.receiverId];
                                }
                                return prevItems;
                              });
                            }}
                ...
                {/* form for creating group and adding users ID */}
                <form
                  onSubmit={(e) => {
                    e.preventDefault();
                ...
                >
                  <Input
                    ...
                    value={groupName} // Make sure to control the input
                    className="mt-2"
                  />
                  <input type="submit"
                    name="Save"
                  ...
                  <Button
                    name="Cancel"
                    className="bg-red-500 mt-2 hover:bg-red-600" // Darkens the background on hover
                    onClick={() => {
                      setGroupName(''); // Clear the input on cancel
                      setGroupMember([]);
                      setGroupSelection(false);
                    }}
                  />
              ...
              <div className='text-blue-600 underline'>Groups</div>
            )}



            {group.map(({ _id, name,createdAt }) => {
              return (
                <div
                  key={_id}
                  className="flex m-2 border-b border-gray-500 py-2 cursor-pointer hover:bg-gray-200 hover:text-blue-500 hover:scale-105 transform transition-all duration-300"
                  onClick={async () => { await fetchMessages("groupID", "group"); }}
                >
                  <div className="border border-blue-400 p-1 rounded-full">
                    <img src={Avatar} alt="profile" width={50} height={50} className="rounded-full" />
                  </div>
                  <div className="ml-10">
                    ...
                    <p className="text-sm text-gray-500">createdAt :{new Date(createdAt).toLocaleString()}</p>
                  </div>
                </div>
              );
            })}
